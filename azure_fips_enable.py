#!/usr/bin/env python3
"""
Enable FIPS-CC mode on an Azure VM-Series firewall via the Maintenance Recovery Tool (MRT).

Workflow:
  1. SSH to the firewall as the deployment user, trigger maintenance mode reboot.
  2. Wait for the MRT to become accessible via SSH (deployment credentials).
...
quiescing and capturing a custom image via vmseries-custom create-custom-image.

# TODO: Add FIPS-CC enablement as an optional post-upgrade step in
# vmseries-custom's create-custom-image workflow, so a specific PAN-OS version
# not available in the Marketplace can be built into a FIPS-CC golden image.

IMPORTANT — SSH KEY REQUIRED:
  Azure VMs deployed with password-only authentication will be permanently
  locked out after the FIPS-CC factory reset. Password auth does not survive
  the factory reset on Azure. You MUST have an SSH key deployed on the
  firewall before enabling FIPS-CC mode.
...
def _state_path(ip: str, state_dir: Path) -> Path:
    return state_dir / f"azure_fips_{ip.replace('.', '_')}.json"
...
def phase_wait_for_post_fips(ip: str, key_path: Path,
    ...
    On Azure, password auth does not survive the factory reset. The SSH key
...
def detect_state(ip: str, admin_user: str, key_path: Path | None,
    ...
    # For FIPS-selected/complete/rebooting states, check if post-FIPS or MRT
...
def enable_fips(ip: str, admin_user: str, key_path: Path | None,
    ...
    if status == STATE_REBOOTING:
        if not key_path:
            LOGGER.error(
                "No SSH key available for post-FIPS reconnect. "
                "Azure password auth does not survive the factory reset. "
...
def main():
    parser = argparse.ArgumentParser(
        description="Enable FIPS-CC mode on an Azure VM-Series firewall via the MRT.",
        formatter_class=argparse.RawTextHelpFormatter,
        epilog="""
Examples:
  python azure_fips_enable.py 10.0.0.100
  python azure_fips_enable.py 10.0.0.100 --mrt-password 'DeploymentP@ss'
  python azure_fips_enable.py 10.0.0.100 --debug

IMPORTANT: The VM must have been deployed with SSH key authentication.
...
WARNING: Enabling FIPS-CC mode performs a full factory reset.
    ...
    parser.add_argument(
        ...
        metavar="USER",
        help=f"Deployment username (default: {DEFAULT_ADMIN_USER})",
    )
    parser.add_argument(
        "--admin-password",
        metavar="PASSWORD",
    ...
    if not key_path.exists():
        ...
        LOGGER.error("An SSH key is required — Azure password auth does not "
                     "survive the FIPS-CC factory reset.")
        sys.exit(1)
